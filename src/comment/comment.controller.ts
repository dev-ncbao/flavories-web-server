...
export class CommentController {
    ...
    @Get()
    @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Number of comments per page (default: 10)' })
    ...
    @ApiQuery({ name: 'parentId', required: false, type: Number, description: 'Filter by parent comment ID (null for top-level)' })
    async getComments(
        ...
        const options = {
            ...
            parentId: parentId ? (parentId === 'null' ? null : parseInt(parentId, 10)) : undefined
        };
    ...
    async getCommentById(@Param('id') id: string): Promise<CommentDto> {
        const comment = await this.commentService.getCommentById(parseInt(id, 10));
    ...
    @Get(':id/replies')
    @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Number of replies (default: 10)' })
    async getReplies(
        @Param('id') id: string,
        @Query('limit') limit?: string
    ): Promise<CommentDto[]> {
        const limitNumber = limit ? parseInt(limit, 10) : 10;
        const replies = await this.commentService.getReplies(parseInt(id, 10), limitNumber);
    ...
    async updateComment(
        ...
        const comment = await this.commentService.updateComment(parseInt(id, 10), updateCommentDto);
